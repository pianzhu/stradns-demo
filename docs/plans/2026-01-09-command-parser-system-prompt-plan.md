...
- NAME
  - 点名设备：输出设备名（如 顶灯、老伙计）
  - 仅按类型泛指（如“灯/空调/风扇/窗帘”等且无具体名称）：输出类型中文名（不要输出 `*`），并与 `TYPE` 一致（如 `Light→灯`、`AirConditioner→空调`、`Blind→窗帘`）
...
- Q（量词，必须输出）
  ...
  - 映射规则：
    ...
    - 其他情况 → `one`
  - 默认规则（已确认）：当用户按类型泛指且未明确量词时，`Q` 默认输出 `all`
- N（数量，可选）
...
>   - NAME：设备名；若用户仅按类型泛指且无具体名称时，NAME 输出类型中文名（不要输出 *），并与 TYPE 一致；指代“它/那个/上一个/刚才的”输出 @last。NAME 不得包含 # 或 -（遇到则用空格替换）。
...
| 打开客厅的灯 | ["打开-客厅-灯#Light#all"] |
| 打开卧室顶灯调到50% | ["打开-卧室-顶灯#Light#one", "设置亮度=50%-卧室-顶灯#Light#one"] |
| 打开卧室顶灯然后关闭客厅灯 | ["打开-卧室-顶灯#Light#one", "关闭-客厅-灯#Light#all"] |
| 打开除卧室以外的灯 | ["打开-*,!卧室-灯#Light#except"] |
