...
智能家居设备上下文检索系统：先用**大模型**将用户请求解析为结构化 QueryIR（动作、量词、排除、指代、条件等），再基于设备元数据（`type`/`room`/`name`）与动作-能力匹配，在本地设备列表中召回并筛选最少且必要的实体用于 prompt 注入。
...
- 给定：用户查询"如果室温超过26度就打开空调"
- 当：进行 LLM 语义解析
- 则：conditions 应包含 temperature 条件
- 且：条件阈值应为 26（摄氏度）
...
> **状态**：待优化。当前未实现，整体方案待设计。

当 QueryIR 包含条件（例如温度/湿度/亮度阈值），系统应将条件相关的传感器设备纳入候选集合，以便 Agent 具备做条件判断所需的上下文。

#### 场景：温度条件触发传感器依赖

- 给定：用户查询"如果卧室温度超过26度就打开空调"
- 且：设备列表包含卧室空调与卧室温度传感器
- 当：进行上下文检索
- 则：候选应包含卧室空调
- 且：候选应包含卧室温度传感器
