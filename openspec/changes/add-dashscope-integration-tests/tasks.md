...
## 1. LLM 解析集成测试
- [x] 准备真实中文 query 用例集（默认不少于 30 条），为每条用例定义最小期望字段（action、name_hint/scope/type_hint/quantifier 等）
- [x] 编写 dashscope LLM 解析用例（qwen-flash），校验输出字段命中预期；并记录 action 非空比例作为覆盖率指标
...
- [x] 先调用 LLM 得到 QueryIR，优先使用 `QueryIR.action`（为空则 fallback 用原始 query）作为 embedding 查询文本
...
**备注**：action.kind 已移除，原 action.kind 指标不再适用；需重新运行以更新 action 覆盖率、召回指标与端到端指标。
...
| 指标 | 结果 | 阈值 | 状态 |
|------|------|------|------|
| action → top-10 命中率 | 90.00% (9/10) | ≥60% | ✅ 通过 |
...
|------|------|
| action 覆盖率 | （待更新） |
| action fallback 率 | （待更新） |
| top-10 召回命中率 | （待更新） |

### 关键发现

1. **action 召回优于 raw query**：90% vs 60%，验证了 LLM 提取意图短语的价值
