...
- 决策：当用户“按类型泛指”且未明确量词时，`NAME` 输出类型中文名（不要输出 `*`），且 `Q` 默认输出 `all`
  - 原因：减少小模型对“通配符边界”的判断负担；同时保持输出自然语言可读，消费侧仍可用 `TYPE+Q` 做确定性解释。
...
- 当用户给出数量但目标是“点名具体设备”（NAME 为具体设备名而非类型词）时，是否应忽略 N 还是输出并由消费侧报错？
