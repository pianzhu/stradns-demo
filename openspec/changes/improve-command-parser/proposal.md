...
- 输出协议升级为：**始终输出** `JSON array<string>`；即使只有一条命令也必须用数组包装。
- 每条命令仍保持三段式：`ACTION-SCOPE-TARGET`，以保持对既有消费侧三段式解析逻辑的友好性。
- 在 `TARGET` 内引入二级分隔槽位：`NAME#TYPE#Q[#N]`，最小化承载 `type（闭集）/量词/数量/指代` 等语义信息。
- 明确多命令拆分规则：多动作、多目标、先后顺序必须拆分，并保持原句顺序输出。
- 规定解析失败的统一降级输出：`["UNKNOWN-*-*#Unknown#one"]`。
- **重大变更**：消费侧不再接受“裸字符串”作为 LLM 输出；必须支持 JSON 数组（短期可先降级只取第 1 条命令）。

## 非目标

- 不引入更复杂的嵌套 JSON 对象结构（例如每条命令输出 object）。
