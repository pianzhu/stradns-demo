...
本节用于把 skills（位于 `~/.codex/skills/`）的工作方式与本文后续的 Plan/Code 工作流对齐，并解决“skills 尚未可用时如何启动”的冲突。

### A. Skills 位置（优先）
- skills 位于 `~/.codex/skills/<skill-name>/SKILL.md`，以本地版本为准（不要凭记忆复述）。
- 任何回复/澄清/动作都必须遵循下一节的 skill-first 规则。

### B. Skill-First 规则（1% 原则）

- 只要有 1% 的可能某个 skill 适用，就必须先使用该 skill，再做任何回复或动作（包括澄清问题）。
- 若使用后发现不适用，可以停止并继续。
- 不要用“这只是个小问题/我先看一下代码/我先跑一个命令”来跳过 skill 检查。
- 用户指令更多描述 WHAT，不等于允许跳过 HOW（skills/workflow）。

### C. 与 Plan 模式的默认组合（推荐）

- 新功能 / 行为变更 / 需求不够明确：先用 `brainstorming` 对齐意图、约束与验收标准，再进入 Plan 模式收敛方案。
- 多步骤实现（涉及多个文件/模块/需要测试与验证策略）：在进入 Code 模式前用 `writing-plans` 产出可执行计划，保存到 `docs/plans/YYYY-MM-DD-<topic>.md` 并等待 review。
...
1. 自上而下分析问题，尽量找出根因和核心路径，而不是只对症状打补丁(可通过deep-analysis辅助分析)。
...
* 如果任务属于多步骤（涉及多个文件/模块、需要测试策略、需要分批验证），你必须先使用 `writing-plans` 产出可执行实现计划（写入 `docs/plans/YYYY-MM-DD-<topic>.md`）并请求 review；在计划被确认后再进入 Code 模式按计划实施；
* 否则（小范围实现、步骤很少），你必须在 **下一条回复中直接进入 Code 模式**，并按选定方案实施；
